...
import GameCategoryFormModal from './components/GameCategoryFormModal'
...
function GameCategory() {
  ...
  const searchGames =() => {
    if (searchInput !== "") {
      navigate(`/admin/game-category?page=1&q=${searchInput}`)
    } else {
      navigate(`/admin/game-category?page=1`)
    }
  };
  ...
  const handlePageChange = (page) => {
    if (searchInput !== "") {
      navigate(`/admin/game-category?page=${page}&q=${searchInput}`)
    } else {
      navigate(`/admin/game-category?page=${page}`)
    }
  };
  ...
  const openModal = (action, obj = null) => {
    ...
    document.getElementById('GameCategoryFormModal').showModal();
  };

  const closeModal = () => {
    document.getElementById('GameCategoryFormModal').close(); // Close the modal
  };
  ...
  const addCategory = useCallback(async (obj) => {
    if (obj.name.trim() === "") {
      ...
      try {
        let response = await axios.post("/admin/game-category/add", obj);
  ...
  const editCategory = useCallback(async (obj) => {
    if (obj.name.trim() === "") {
      ...
      try {
        let response = await axios.put(`/admin/game-category/edit/${obj._id}`, obj);
        response = response.data;
        if (response.success) {
  ...
  const deleteCategory = useCallback(async (obj) => {
    try {
      let response = await axios.delete(`/admin/game-category/delete/${obj._id}`, { data: obj });
      response = response.data;
      if (response.success) {
        getGameCategories();
        closeModal();
      ...
      if (errorResponse.message === "This category contains some games") {
        closeModal();
  ...
  const getGameCategories = useCallback(async () => {
    ...
    try {
      ...
      response = response.data;
      if (response.success) {
        const { gameCategories, totalPages } = response.data;
  ...
  return (
    <div>
      {/* Modal */}
      <GameCategoryFormModal
        ModalId={"GameCategoryFormModal"}
      ...
      <div className='mb-4 w-full flex justify-between'>
        ...
        <div className='flex gap-4'>
          <label className="input input-bordered flex items-center gap-2">
            ...
            <kbd className="kbd kbd-sm" onClick={searchGames}>
        ...
        <div className="card-body" style={{ position: "relative" }}>
          <div className="overflow-x-auto w-full ">
            <Show>
              ...
              <Show.Else>
                <div className='m-4'>No games categories to show.</div>
              </Show.Else>
            </Show>
