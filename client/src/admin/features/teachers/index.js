...
const Teachers = () => {
    ...
    const searchTeachers = () => {
        if (searchInput !== "") {
            navigate(`/admin/teachers?page=1&q=${searchInput}`)
        } else {
            navigate(`/admin/teachers?page=1`)
        }
    };

    const handlePageChange = (page) => {
        if (searchInput !== "") {
            navigate(`/admin/teachers?page=${page}&q=${searchInput}`)
        } else {
            navigate(`/admin/teachers?page=${page}`)
        }
    };
    ...
    const getTeachers = useCallback(async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                const { teachers, totalPages } = response.data;
            ...
            <div className='card  w-full bg-base-100 shadow-xl h-[80vh]'>
                <div className="card-body" style={{ position: "relative" }}>
                    <div className="overflow-x-auto w-full">
                        <Show>
                            <Show.When isTrue={teachers.length > 0}>
                                <table className="table">
                                    <thead>
                                        <tr>
                                            <th className="w-1/5">Name</th>
                                            <th className='w-1/4'>Bio</th>
                                            <th className="w-1/5">Phone</th>
                                            <th className="w-1/5">Email</th>
                                            <th className="w-2/5">Joining Date</th>
                                            <th className="w-1/5">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {teachers.map((l, k) => {
                                            const profileUrl = l?.profile ? `${process.env.REACT_APP_STORAGE_BASE_URL}/teacher/profile/${l.profile}` : defaultProfile
                                            return (
                                                <tr key={k}>
                                                    <td>
                                                    ...
                                                    <td>{l.bio?.length > 40 ? l.bio.substring(0, 40) + "..." : l.bio || <div className='w-100 ml-8'>—</div>}</td>
                                                    <td>{l.phone || <div className='w-100 ml-8'>—</div>}</td>
                                                    <td>{l.email || <div className='w-100 ml-8'>—</div>}</td>
                                                    <td>{formatDate(l.createdAt)}</td>
                                                    <td>
                                                        <button className="btn btn-ghost" onClick={() => navigate(`/admin/teachers/${l._id}`)}>
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                            ...
                            <Show.Else>
                                <div className='m-4'>No teachers to show.</div>
                            </Show.Else>
                        </Show>
