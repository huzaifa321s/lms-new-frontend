...
function ResetPassword() {
    ...
    const postData = async (obj) => {
        try {
            let response = await axios.post(`/admin/resetPassword/${id}/${token}`, obj);
            response = response.data;

            if(response.success) navigate('/admin/login');
                ...
                <div className="bg-base-100 rounded-xl">
                    <div className='py-24 px-10'>
                        ...
                        <form onSubmit={(e) => submitForm(e)}>
                            ...
                            <div className='text-center mt-4'>Don't have an account yet? <Link to="/admin/register"><span className="  inline-block  hover:text-primary hover:underline hover:cursor-pointer transition duration-200">Register</span></Link></div>
                        </form>
                    </div>
                </div>
            </div>
