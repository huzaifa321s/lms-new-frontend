...
import { handleLogin } from './teacherAuthSlice'
...
function Login() {
    ...
    const postData = async (obj) => {
        try {
            let response = await axios.post("/teacher/login", obj);
                    ...
                    <div className='py-24 px-10'>
                        ...
                        <form onSubmit={(e) => submitForm(e)}>
                            ...
                            <div className='text-right text-primary'>
                                <Link to="/teacher/forgot-password"><span className="text-sm inline-block blue-link">Forgot Password?</span></Link>
                            </div>

                            <ErrorText styleClass="mt-8">{errorMessage}</ErrorText>
                            <button type="submit" className={"btn my-2 w-full gradiant-btn" + (loading ? "loading" : "")}>Login</button>

                            <div className='text-center mt-4'>Don't have an account yet? <Link to="/teacher/register"><span className="inline-block blue-link">Register</span></Link></div>
                        </form>
                    </div>
                </div>
            </div>
