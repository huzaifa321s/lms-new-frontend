...
import { handleLogin } from './teacherAuthSlice';import { useDispatch } from 'react-redux'
   ...
   const dispatch = useDispatch();
    ...
    const validateForm = () => {
        ...
        // Validation for email
            ...
            isValid = false;
        } else if (!/\S+@\S+\.\S+/.test(registerObj.email)) {
    ...
    const postData = async (formData) => {
        try {
            let response = await axios.post("/teacher/register", formData);
    ...
    const submitForm = (e) => {
        ...
        if (isValid) {
            ...
            for (const key in registerObj) {
                registrationForm.append(key, registerObj[key]);
            }
            ...
            <div className="card mx-auto w-full max-w-5xl  shadow-xl">
                <div className="grid  md:grid-cols-2 grid-cols-1  bg-base-100 rounded-xl">
                    ...
                    <div className='py-24 px-10'>
                        <h2 className='text-2xl font-semibold mb-2 text-center'>Teacher Register</h2>
                        <form onSubmit={(e) => submitForm(e)}>
                            ...
                            <div className='text-center mt-4'>Already have an account? <Link to="/teacher/login"><span className="inline-block blue-link">Login</span></Link></div>
                        </form>
                    </div>
                </div>
            </div>
