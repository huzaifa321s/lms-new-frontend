...
import { createNewSentenceArray } from '../../../../teacher/utils/helperFunctions';
...
const TrainingWheelGameForm = () => {
    ...
    const addQuestion = useCallback(async (action = 'add') => {
        ...
        try {
            let response = await axios.post("/admin/game/training-wheel-game/create", reqBody);
            response = response.data;
            if (response.success) {
                toast.success(response.message);
                // Add
                if (action === 'add') {
                    navigate('/admin/training-wheel-game');
                }
                // Add and New
    ...
    const updateQuestion = useCallback(async () => {
        ...
        try {
            let response = await axios.put(`/admin/game/training-wheel-game/update/${id}`, reqBody);
            response = response.data;
            if (response.success) {
                toast.success(response.message);
                navigate('/admin/training-wheel-game');
    ...
    const getTWGame = useCallback(async () => {
        try {
            let response = await axios.get(`/admin/game/training-wheel-game/get-game/${id}`);
            response = response.data;
    ...
    const getGameCategories = useCallback(async () => {
        try {
            let response = await axios.get("/admin/game-category/getAll");
            response = response.data;
            if (response.success) {
    ...
    return (
        <div>
            ...
            <div className='card w-full p-6 bg-base-100 shadow-xl mt-2'>
                ...
                <div className="mb-3 flex gap-4">
                    <div className="form-control w-full">
                        <label className="label label-text text-base-content"> Answer </label>
                        <textarea
                            type="text"
