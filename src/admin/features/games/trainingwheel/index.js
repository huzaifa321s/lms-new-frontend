...
const TrainingWheelGame = () => {
    ...
    const searchQuestions =() => {
        if (searchInput !== "") {
            navigate(`/admin/training-wheel-game?page=1&q=${searchInput}`)
        } else {
            navigate(`/admin/training-wheel-game?page=1`)
        }
    };

    const handlePageChange = (page) => {
        if (searchInput !== "") {
            navigate(`/admin/training-wheel-game?page=${page}&q=${searchInput}`)
        } else {
            navigate(`/admin/training-wheel-game?page=${page}`)
        }
    };
    ...
    const getQuestions = useCallback(async () => {
        ...
        try {
            let response = await axios.get(`/admin/game/training-wheel-game/get?${queryStr}`);
    ...
    const deleteGame = useCallback(async () => {
        try {
            let response = await axios.delete(`/admin/game/training-wheel-game/delete/${questionId}`);
            response = response.data;
            if (response.success) {
                getQuestions();
            }
        ...
        <div>
            ...
            <div className='mb-4 w-full flex justify-between'>
                ...
                <div className='flex gap-4'>
                    ...
                    <button className="btn btn-active" onClick={() => navigate('/admin/training-wheel-game/create')}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                            ...
                            <Show.When isTrue={questions.length > 0}>
                                <table className="table w-full">
                                    ...
                                    <tbody>
                                        {
                                            questions.map((l, k) => {
                                                console.log('l.levels ---> ', l.difficulties)
                                                return (
                                                    <tr key={k}>
                                                        <td>{l.question}</td>
                                                        <td>{l.author.firstName} {l.author.lastName}</td>
                                                        ...
                                                        <td className='flex gap-1'>
                                                            {l.difficulties.map((d, i) => {
                                                                ...
                                                                return <div key={i} className={`badge ${color} p-2 badge-xs`}>{d}</div>
                                                            })}
                                                        </td>
                                                        <td className='text-right'>
                                                            ...
                                                            <button className='btn btn-ghost btn-xs' onClick={() => navigate(`/admin/training-wheel-game/edit/${l._id}`)}  >
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
