...
function BlogCategory() {
  ...
  const searchBlogs = () => {
    if (searchInput !== "") {
      navigate(`/admin/blog-category?page=1&q=${searchInput}`)
    } else {
      navigate(`/admin/blog-category?page=1`)
    }
  };
  ...
  const handlePageChange = (page) => {
    if (searchInput !== "") {
      navigate(`/admin/blog-category?page=${page}&q=${searchInput}`)
    } else {
      navigate(`/admin/blog-category?page=${page}`)
    }
  };
  ...
  const openModal = (action, obj = null) => {
    ...
    document.getElementById('BlogCategoryFormModal').showModal();
  };

  const closeModal = () => {
    document.getElementById('BlogCategoryFormModal').close(); // Close the modal
  };
  ...
  const addCategory = useCallback(async (obj) => {
    if (obj.name.trim() === "") {
      ...
      try {
        let response = await axios.post("/admin/blog-category/add", obj);
  ...
  const editCategory = useCallback(async (obj) => {
    if (obj.name.trim() === "") {
      ...
      try {
        let response = await axios.put(`/admin/blog-category/edit/${obj._id}`, obj);
        response = response.data;
        if (response.success) {
  ...
  const deleteCategory = useCallback(async (obj) => {
    try {
      let response = await axios.delete(`/admin/blog-category/delete/${obj._id}`, { data: obj });
      response = response.data;
      if (response.success) {
        getBlogCategories();
        closeModal();
      ...
      if (errorResponse.message === "This category contains some blogs") {
        closeModal();
  ...
  const getBlogCategories = useCallback(async () => {
    ...
    try {
      ...
      response = response.data;
      if (response.success) {
        const { blogCategories, totalPages } = response.data;
